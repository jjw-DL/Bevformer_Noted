...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def get_reference_points(H, W, Z=8, num_points_in_pillar=4, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        if dim == '3d':
            # 0.5~7.5中间均匀采样4个点 (4,)-->(4, 1, 1)-->(4, 150, 150) 归一化
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        # reference_points:(1, 4, 22500, 3)
        # 1.由归一化参考点-->lidar系下的实际参考点-->齐次坐标
        lidar2img = []
        for img_meta in img_metas:
            lidar2img.append(img_meta['lidar2img'])
        lidar2img = np.asarray(lidar2img) # (1, 6, 4, 4)
        lidar2img = reference_points.new_tensor(lidar2img)  # (1, 6, 4, 4) 将numpy.array转化为tensor
        ...
        D, B, num_query = reference_points.size()[:3] # 4, 1, 22500
        num_cam = lidar2img.size(1) # 6
        # (4, 1, 1, 22500, 4)-->(4, 1, 6, 22500, 4)-->(4, 1, 6, 22500, 4, 1)
        reference_points = reference_points.view(
            D, B, 1, num_query, 4).repeat(1, 1, num_cam, 1, 1).unsqueeze(-1)
        # (1, 6, 4, 4) --> (1, 1, 6, 1, 4, 4)-->(4, 1, 6, 22500, 4, 4)
        ...
        reference_points_cam = reference_points_cam[..., 0:2] / torch.maximum(
            reference_points_cam[..., 2:3], torch.ones_like(reference_points_cam[..., 2:3]) * eps) # (4, 1, 6, 22500, 2)
        ...
        bev_mask = (bev_mask & (reference_points_cam[..., 1:2] > 0.0)
                    ...
                    & (reference_points_cam[..., 0:1] > 0.0)) # (4, 1, 6, 22500, 1)
        ...
        reference_points_cam = reference_points_cam.permute(2, 1, 3, 0, 4) # (6, 1, 22500, 4, 2) 以相机为单位
    ...
    def forward(self,
        ...
        output = bev_query # (22500, 1, 256)
        intermediate = [] # 初始化中间值list

        # (1, 4, 22500, 3) 归一化3d参考点
        ref_3d = self.get_reference_points(
            bev_h, bev_w, self.pc_range[5]-self.pc_range[2], self.num_points_in_pillar, dim='3d', bs=bev_query.size(1),  device=bev_query.device, dtype=bev_query.dtype)
        # (1, 22500, 1, 2) BEV平面归一化2d参考点
        ref_2d = self.get_reference_points(
            bev_h, bev_w, dim='2d', bs=bev_query.size(1), device=bev_query.device, dtype=bev_query.dtype)
        # 3d参考点的主要作用是获取相机参考点和有效bev的mask
        reference_points_cam, bev_mask = self.point_sampling(
            ref_3d, self.pc_range, kwargs['img_metas']) # (6, 1, 22500, 4, 2)和 (6, 1, 22500, 4)
        
        # bug: this code should be 'shift_ref_2d = ref_2d.clone()', we keep this bug for reproducing our results in paper.
        shift_ref_2d = ref_2d #.clone() # 获取BEV平面上的2D参考点
        shift_ref_2d[..., :] += shift # 在原始参考点的基础上+偏移量 [0.002, 0.045]
        ...
        if prev_bev is not None:
            ...
            prev_bev = torch.cat([prev_bev, bev_query], 0) # (2, 22500, 256) # T-1帧BEV和T帧BEV拼接
            hybird_ref_2d = torch.cat([shift_ref_2d, ref_2d], 0) # (2, 22500, 1, 2)
        else:
            hybird_ref_2d = torch.cat([ref_2d, ref_2d], 0) # (2, 22500, 1, 2)
...
class BEVFormerLayer(MyCustomBaseTransformerLayer):
    ...
    def __init__(self,
        ...
        super(BEVFormerLayer, self).__init__(
            attn_cfgs=attn_cfgs,
            feedforward_channels=feedforward_channels, # 512
            ffn_dropout=ffn_dropout, # 0.1
            operation_order=operation_order, # ('self_attn', 'norm', 'cross_attn', 'norm', 'ffn', 'norm')
            act_cfg=act_cfg, # dict(type='ReLU', inplace=True)
            norm_cfg=norm_cfg, # dict(type='LN')
        ...
        for layer in self.operation_order:
            ...
            # spaital cross attention
                ...
                query = self.ffns[ffn_index](
                    query, identity if self.pre_norm else None) # (1, 22500, 256)
                ffn_index += 1
